...
export function getPm25Status(pm2_5: number): { text: string; color: string; icon: string } {
  if (pm2_5 <= 8) {
    ...
    return { text: '좋음', color: '#4D97CF', icon: `${icons.air.lv1}` };
  } else if (pm2_5 <= 20) {
    return { text: '보통', color: '#50AABF', icon: `${icons.air.lv1}` };
  } else if (pm2_5 <= 25) {
    return { text: '나쁘지 않음', color: '#4E8E4B', icon: `${icons.air.lv1}` };
  } else if (pm2_5 <= 37) {
    return { text: '나쁨', color: '#E8923D', icon: `${icons.air.lv1}` };
  } else if (pm2_5 <= 50) {
    return { text: '상당히 나쁨', color: '#D65835', icon: `${icons.air.lv1}` };
  } else if (pm2_5 <= 75) {
    return { text: '실외활동 자제요망', color: '#C43E38', icon: `${icons.air.lv1}` };
  } else {
    return { text: '외출시 사망확률 높음', color: '#C43E38', icon: `${icons.air.lv1}` };
  }
}
